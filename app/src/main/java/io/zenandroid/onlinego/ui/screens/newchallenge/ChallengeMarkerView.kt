...
class ChallengeMarkerView(context: Context, onProfile: (Player) -> Unit, onAccept: (Long) -> Unit) : ClickableMarkerView(context, R.layout.challenge_markerview) {
    ...
    private val currentRating = get().get<UserSessionRepository>().userRating ?: 0

    var challenge: SeekGraphChallenge? = null
    ...
    override fun refreshContent(e: Entry, highlight: Highlight) {
        (e.getData() as? SeekGraphChallenge)?.let { challenge = it }
        challenge?.let {
            ...
            acceptButton.setVisibility(when {
                it.ranked && (e.getY() - currentRating) > 9 -> View.INVISIBLE
                it.min_rank > currentRating -> View.INVISIBLE
                it.max_rank < currentRating -> View.INVISIBLE
                else -> View.VISIBLE
            })
            rankTextView.setText("${it.username} [${formatRank(it.rank)}]")
            tpmTextView.setText("~$timePerMove / move")
            userTextView.setText("\"${it.name}\": $ranked $size, $handicap, $params\nRanks: $ranks")
        }
    ...
    override fun getOffset(): MPPointF {
        return MPPointF(-(getWidth().toFloat() / 2), -getHeight().toFloat() - 10)
    }
}
